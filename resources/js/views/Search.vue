<template>
	<div>
		...
		<div v-else class="p-search c-inner--l">
			<!-- カルーセル -->
			...
			<div class="p-search__container">
				...
				<div class="p-search__main">					
							...
							<div class="c-jobList__unit">
								...
								<div class="c-jobList__base">
									...
									<div class="c-jobList__btnArea">
										<!-- 未応募の求人で「気になるボタン」を表示 -->
										<bookmark-btn-component v-if="showBookmarkBtn(job.id)" :id="job.id" class="c-jobList__btn"></bookmark-btn-component>
					...
					<div v-else class="c-searchNaviArea--zeroAnnounce">
						<p class="c-searchNaviArea__zeroAnnounceCopy">条件にあてはまる求人情報がありません。</p>
						<p class="c-searchNaviArea__zeroAnnounceText">検索条件を広げて検索してみてください。<br>新たに転職情報を発見できるチャンスです!!</p>
...
export default {
	...
	computed: {
		// 気になるボタンの表示判定(応募済み求人は非表示)
		showBookmarkBtn: function() {
			return function(id) {
				return !this.applyIDs.includes(id) && this.isDataAcquired;
			}
		},
		...
		// 応募済み求人のID配列
		applyIDs: function () {
			return this.$store.state.Pagination.applyIDs;
		},
