...
const mutations = {
	...
	// 検索ページではヘッダーのリンク無効
	setIsSearched(state, bool){
		state.isSearched = bool;
	},
	// 応募済み求人のID配列
	setApplyIDs(state, array){
		state.applyIDs = array;
	},
	// 検索ボタンのクリック判定
	setIsClickBtn(state, bool){
		state.isClickBtn = bool;
	},
}
	...
	getJobs: function (context) {
		...
		return axios.post('/job20/web/index', {
			...
			if(response.data.successFlg){
				...
				context.commit('setApplyIDs', response.data.applyIDs);
